...
steps:
  picard_collectaggregationmetrics:
    run: ../agents_withsrc/picard_collectaggregationmetrics.cwl
    in:
  ...
  picard_collectreadgroupbamqualitymetrics:
    run: ../agents_withsrc/picard_collectreadgroupbamqualitymetrics.cwl
    in:
      input_bam: picard_gatherbamfiles_output
      reference: indexed_reference_fasta
    out: [output1, output2]
  ...
  picard_collectwgsmetrics:
    run: ../agents_withsrc/picard_collectwgsmetrics.cwl
    in:
      input_bam: picard_gatherbamfiles_output
      reference: indexed_reference_fasta
  ...
  samagents_coverttocram:
    run: ../agents_withsrc/samagents_covert_to_cram.cwl
    in:
      input_bam: picard_gatherbamfiles_output
      reference: indexed_reference_fasta
    out: [output]
  ...
  picard_intervallistagents:
    run: ../agents_withsrc/picard_intervallistagents.cwl
    in:
      interval_list: wgs_calling_interval_list
    out: [output]

  checkcontamination:
    run: ../agents_withsrc/expression_checkcontamination.cwl
    in: 
      verifybamid_selfsm: verifybamid_selfsm
    out: [contamination]

  gatk_haplotypecaller:
    run: ../agents_withsrc/gatk_haplotypecaller.cwl
    in:
      reference: indexed_reference_fasta
      input_bam: picard_gatherbamfiles_output
  ...
  picard_mergevcfs:
    run: ../agents_withsrc/picard_mergevcfs.cwl
    in:
  ...
  picard_collectgvcfcallingmetrics:
    run: ../agents_withsrc/picard_collectgvcfcallingmetrics.cwl
    in:
  ...
  gatk_validategvcf:
    run: ../agents_withsrc/gatk_validategvcf.cwl
    in:
      input_vcf: picard_mergevcfs/output
      reference: indexed_reference_fasta
