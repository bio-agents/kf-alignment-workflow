...
steps:
  getbasename:
    run: ../agents/expression_getbasename.cwl
    in:
      input_file: input_cram
    out: [file_basename]

  picard_revertsam:
    run: ../agents/picard_revertsam_inputcram.cwl
    in:
  ...
  gatk_haplotypecaller:
    run: ../agents/gatk_haplotypecaller_35.cwl
    in:
      reference: indexed_reference_fasta
      input_bam: picard_gatherbamfiles/output
  ...
  gatk_validategvcf:
    ...
    in:
      input_vcf: picard_mergevcfs/output
      reference: indexed_reference_fasta
      wgs_calling_interval_list: picard_intervallistagents/output
      dbsnp_vcf: dbsnp_vcf
