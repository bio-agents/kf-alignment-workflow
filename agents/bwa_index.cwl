...
id: bwa_index
doc: |-
  This agent conditionally generates the bwa 64 indexes for an input fasta file using bwa index.
  The agent will generate the indexes only of generate_bwa_indexes is set to true AND any of the alt,
  amb, ann, bwt, pac, or sa files is missing.
  The agent returns the six indexes as its output.
requirements:
